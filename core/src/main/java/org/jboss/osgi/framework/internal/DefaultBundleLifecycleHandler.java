...
class DefaultBundleLifecycleHandler implements BundleLifecyclePlugin.DefaultHandler {
    ...
    public void start(XBundle bundle, int options) throws BundleException {
        ...
        try {
            ...
            try {
                hostState.changeState(AbstractBundleState.STARTING);
            ...
            if (className != null) {
                ...
                // If the BundleActivator is invalid or throws an exception then
                    ...
                    hostState.changeState(AbstractBundleState.STOPPING);
                    ...
                    hostState.changeState(AbstractBundleState.RESOLVED);
            ...
            if (hostState.getState() == AbstractBundleState.UNINSTALLED)
            ...
            hostState.changeState(AbstractBundleState.ACTIVE);

            // Activate the service that represents bundle state ACTIVE
            hostState.getBundleManager().setServiceMode(hostState.getServiceName(AbstractBundleState.ACTIVE), Mode.ACTIVE);
    ...
    public void stop(XBundle bundle, int options) throws BundleException {
        ...
        try {
            ...
            if ((options & AbstractBundleState.STOP_TRANSIENT) == 0) {
            ...
            if (priorState != AbstractBundleState.STARTING && priorState != AbstractBundleState.ACTIVE)
                return;
            ...
            if (priorState == AbstractBundleState.ACTIVE) {
                if (hostState.bundleActivator != null) {
                    try {
            ...
            hostState.changeState(AbstractBundleState.RESOLVED, BundleEvent.STOPPED);

            // Deactivate the service that represents bundle state ACTIVE
            hostState.getBundleManager().setServiceMode(hostState.getServiceName(AbstractBundleState.ACTIVE), Mode.NEVER);
